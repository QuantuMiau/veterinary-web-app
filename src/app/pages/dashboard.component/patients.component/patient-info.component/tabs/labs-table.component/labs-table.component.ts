...
@Component({
  ...
  styleUrl: './labs-table.component.css',
})
export class LabsTableComponent implements OnInit, OnChanges {
  ...
  loadServices() {
    this.serviceService.getAll().subscribe({
      next: (svs) => {
        this.allServices = svs.filter(s => s.active && s.service_type === 'lab');
  ...
  load() {
    ...
    this.recordService.getByPatient(this._patientId).subscribe({
      next: (all) => {
        this.records = all.filter(r => r.category === 'laboratorio');
